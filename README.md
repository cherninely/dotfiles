...
Пока здесь будет инфа об основных мапах и плагинах, которые отличают этот `.vimrc` от остальных.

##### [NeoBundle](https://github.com/Shougo/neobundle.vim) - менеджер плагинов для vim.

##### [Unite](https://github.com/Shougo/unite.vim) - оболочка для работы со списками ресурсов в vim.
Название звучит немного абстрактно, поэтому напишу об основных кейсах использования с этим `.vimrc`.

1. `<Space>b` - Показать список открытых файлов.
2. `<Space>f` - Поиск файлов по проекту (в проектах с bem подтормаживает из-за большого количества файлов, это надо пофиксить)
...
Для поиска кода по файловой системе рекомендуется использовать программы ag или ack. По функциональности они одинаковые, но ag работает значительно быстрее. В файле `.ackrc` находятся конфиги, которые учитываются при поиске.
